...
def main_menu():
    ...
    user = auth.login()  # Get user after login
    if not user:  # If user is None (invalid login), exit the program or prompt for re-login
        print("Exiting...")
    ...
    if user.role == "admin":
        ...
        # User has limited access (view only)
        while True:
            print("\nInventory Management System. ")
